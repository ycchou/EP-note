<!-- .slide: class="title-slide" -->

<div style="display:flex; justify-content:center; margin: 0.4em 0 0.6em;">
  <span class="carto3-logo size-xl on-dark" aria-label="CARTO 3 System">
    <span class="ct-row">
      <span class="ct-word">CARTO<span class="ct-tm">™</span></span>
      <span class="ct-three">3</span>
    </span>
    <span class="ct-system">System</span>
  </span>
</div>

<div class="part-no">BIOSENSE WEBSTER · TRAINER EDITION</div>

# VPC Ablation Workflow

<div class="lede">完整 Workflow · 從 Pre-case 到 Post-ablation</div>

<div class="meta">
  <span class="chip">Pattern Matching</span>
  <span class="chip">Auto Pattern Bank</span>
  <span class="chip red">LAT Hybrid</span>
  <span class="chip cyan">PASO</span>
</div>

Note:
這是給 CARTO operator / 廠商工程師的完整 VPC ablation workflow 訓練。內容對應 ycchou/EP-note repo 的 VPC-case-workflow.md，已經整理成 15 個 Part、4 個模組的 SOP。預計 60–80 分鐘。

---

## Agenda · 課程章節

<div class="cols">
<div>

### Part I · EKG 判讀基礎
- 12-Lead ECG 向量基礎（Einthoven）
- VPC Origin 三步驟
- Transition Zone

### Part II · Pre-case &amp; Setup
- 12-lead 預判
- CARTO case setup
- Catheter 配置

### Part III · 目標 VPC morphology
- Pattern Matching
- Auto Pattern Bank
- Anatomy map

</div>
<div>

### Part IV · Mapping &amp; Pacing
- LAT Hybrid
- PASO
- LAT + PASO 結合

### Part V · Ablation
- Target checklist
- During / Post-ablation

### Part VI · 進階臨床情境
- 4 大 Scenarios
- Module timeline
- Decision tree

</div>
</div>

---

## 整堂課要記住的一張圖

<img src="assets/diagrams/workflow-flow.svg" alt="VPC ablation workflow flow diagram" />

Note:
這張圖貫穿整堂課 — 每個 Part 都會回到這個架構。
